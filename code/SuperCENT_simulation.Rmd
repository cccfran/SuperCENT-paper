...
pacman::p_load(data.table, matrixStats, dplyr, ggplot2, ggpubr,
               grid, gridExtra, gtable, latex2exp, facetscales, stringr)
...
knitr::opts_chunk$set(echo = F, cache = T, fig.height = fig_height, fig.width = fig_width)
```
...
# Rank-10 network example

The section for the simulation results of the multi-rank unified framework with a rank-10 network.

```{r}
params <- NULL
params$job_id <- c(1991982, 1745349) # r = 9
params$job_name <- "confint"
...
methods <- c("two_stage", "lr_oracle", "ols", "lr_oracle_oracle")
...
The following chunk produces the plot for the comparison between the two-stage and SuperCENT in terms of the estimations of $u$ and $\beta_u$ as well as the coverage probability and the width of $CI_{\beta_u}$ varying the network noise level $\sigma_a$ and the gap between the leading singular value and the non-leading ones.


```{r fig.height=fig_height}
epsyy <- -2
dds <- c(0, 2^c(-1, -0.5))
ret <- ret[betav == 1 & betau == 16 & log2(epsy) == epsyy]

r <- length(strsplit(ret[1, ds], "_")[[1]])
r_label <- TeX("$d_2=d_3=d_4$: ")
if(r > 4) r_label <- TeX(paste0("$d_2=...$", "$= d_{", r, "}$: "))

ret[, sinu := sqrt(u^2)  ] 
ret[, dd := tstrsplit(ds, "_")[[2]]]
ret[is.na(dd), dd := 0]
ret[, bias_betau := (hbetau - betau)]
...
labs_cols[method == "two_stage", cols := "#F8766D"]

ret_sub <- ret
ret_sub$method <- factor(ret$method, levels = labs_cols$method)

x_breaks <- log2(unique(ret[[xaxis]]))
x_breaks <- x_breaks[order(x_breaks)]
x_breaks <- x_breaks[seq(1, length(x_breaks), 2)]

# get title
# if(is.null(title)) title <- value

toLabel <- function(x) {
  ifelse(x == 0, "0", TeX(paste0("$2^{", 
                                 round(log2(as.numeric(x)), 1), "}$")) )
}
...
p1 <- ret_sub[,
    ...
    by=c("method", "epsy", "epsa", "betau", "betav", "d", "dd")][dd %in% dds & method %in% methods] %>%
  ggplot(aes(x = log2(epsa), 
             y = scale_y_transform(mean_ratio), 
             group = interaction(method, dd), 
             # fill = method,
             col = method,
             linetype =  dd, 
             shape = method)) +
...
This section shows the top 20 squared singular values of the observed network adjacency matrix $A$ with fixed leading singular value $d_1=1$ for the noiseless component $A_0$, fixed noise level $\sigma_a = 2$ and various non-leading singular values for $A_0$.

```{r}
params <- NULL
params$job_id <- c(1992036, 1991851) # r = 9
params$job_name <- "confint"
...
```{r}
ret[, n_test := NULL]
ret[, observed_ratio := NULL]
ret[ds!=1, dd := tstrsplit(ds, "_")[[2]]]
ret[is.na(dd), dd := 0]

r <- length(strsplit(ret[1, ds], "_")[[1]])

ret_long <- melt(ret, id.vars = c("epsy", "epsa", "n", "d", "dd", "s"),
                 measure = patterns("^d_hat"),
                 variable.name = "d_hat")
ret_long[, d_hat := str_replace(d_hat, "d_hat_", "")]
```


```{r}
ret_sub <- ret_long[d_hat %in% 1:20 & log2(epsa) == 2][
  , .(d2 = mean(value^2)), c("epsy", "epsa", "dd", "d_hat")]
ret_sub[dd!=0, dd := round(log2(as.numeric(dd)), 2)]
ret_sub[, dd := factor(dd, levels = c("0", "-1", "-0.5"))]

levels(ret_sub$dd) <- c(`0` = TeX("0"),
                        `-1.0` = TeX("$2^{-1}$"),
                        `-0.5` = TeX("$2^{-0.5}$"))

ret_sub %>%
  ggplot(aes(x = as.numeric(d_hat), y = d2, group = epsa)) + 
  geom_point() +
  # geom_line() +
  theme_bw() +
  ...
  my_theme +
  facet_wrap(~dd, labeller = label_parsed )

ggsave(paste0("../output_simulation/plot/",
              paste(sim_setup$job_id, collapse = "-"), "_epsy",
              epsyy,
              "_small_exmaple_rank_", r, "_d_hat.pdf"), 
       width = 12, height = 3)
```
